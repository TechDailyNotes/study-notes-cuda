...
__global__ void matmul_kernel(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    // Save matmul results to C with vectorized store.
    for (int ti = 0; ti < THREAD_TILE_SIZE_Y; ti++) {
        int C_ri = C_r0 + ti;
        if (C_ri >= M) continue;
        ...
        if (C_ci + 1 < K && reinterpret_cast<uintptr_t>(&C[C_id]) % alignof(float2) == 0) {
            float2 out;
            out.x = acc[ti][0];
            out.y = acc[ti][1];
            *reinterpret_cast<float2*>(&C[C_id]) = out;
        } else {
            if (C_ci < K) C[C_id] = acc[ti][0];
            if (C_ci + 1 < K) C[C_id + 1] = acc[ti][1];
        }
    }
}
