...
bool validate_accuracy(float *C_cpu, float *C_gpu, int m, int n) {
    for (int mi = 0; mi < m; mi++) {
        for (int ni = 0; ni < n; ni++) {
            if (fabs(C_cpu[mi * n + ni] - C_gpu[mi * n + ni]) > 1e-4) {
                printf("Accuracy error.");
                printf("C_cpu = %f, C_gpu = %f.\n", C_cpu[mi * n + ni], C_gpu[mi * n + ni]);
                return false;
            }
        }
    }
...
int main() {
    ...
    if (validate_accuracy(h_C_cpu, h_C_gpu, M, N)) {
        printf("CPU/GPU results match.\n");
    } else {
        printf("CPU/GPU results don't match.\n");
    }
